...
<body class="bg-amber-50 min-h-screen text-slate-800 flex flex-col justify-between">
    <div class="w-full max-w-md mx-auto p-4 flex flex-col min-h-screen justify-between">
        ...
        <div class="grid grid-cols-3 gap-1 mb-3">
            ...
            <button onclick="switchTab('puzzle')" id="tab-puzzle" class="py-2.5 px-1 text-xs rounded-xl font-bold bg-white text-slate-700 border-2 border-amber-200 transition">조각 맞추기</button>
        </div>
        ...
        <main class="flex-1 flex flex-col justify-center space-y-3">
            <div id="view-study" class="space-y-3">
                <div class="bg-white rounded-3xl shadow-md p-6 border-4 border-amber-200 text-center relative overflow-hidden">
                    ...
                    <div id="verse-ref" class="text-xl font-black text-amber-700 mb-3 py-2">고린도후서 5:17</div>
                    
                    <div id="verse-card-box" onclick="toggleVerseContent()" class="min-h-[100px] flex items-center justify-center p-4 bg-amber-50 rounded-2xl border-2 border-dashed border-amber-300 cursor-pointer transition hover:bg-amber-100 mb-3">
                        <p id="verse-text" class="text-sm font-bold text-slate-400">여기를 눌러서 말씀을 확인해보세요</p>
                    </div>

                    <div class="flex justify-center gap-2 mb-4">
                        <button onclick="setVoiceType('normal')" id="v-normal" class="px-4 py-2 rounded-xl text-xs font-bold bg-amber-500 text-white shadow-sm">정상적인 목소리</button>
                        <button onclick="setVoiceType('funny')" id="v-funny" class="px-4 py-2 rounded-xl text-xs font-bold bg-slate-100 text-slate-600">재미있는 목소리</button>
                    </div>
            ...
            <div id="view-puzzle" class="hidden space-y-3">
                <div class="bg-white rounded-3xl shadow-md p-5 border-4 border-emerald-200 text-center">
                    <h2 class="text-xs font-black text-emerald-700 mb-2">말씀 조각을 순서대로 맞추어보세요</h2>
                    <div id="puzzle-target" class="min-h-[70px] bg-emerald-50 border border-dashed border-emerald-300 rounded-xl p-3 flex flex-wrap gap-1.5 mb-3 items-center"></div>
                    <div id="puzzle-source" class="flex flex-wrap gap-1.5 mb-4 max-h-[120px] overflow-y-auto justify-center"></div>
                    <div class="flex gap-2">
                        <button onclick="resetPuzzle()" class="flex-1 py-2.5 bg-slate-100 text-slate-700 rounded-xl text-xs font-bold">처음부터</button>
                        <button onclick="checkPuzzle()" class="flex-2 py-2.5 bg-emerald-600 text-white rounded-xl text-xs font-bold">정답 확인</button>
                    </div>
                    <div id="puzzle-result" class="mt-2 text-xs font-bold hidden"></div>
                </div>
            </div>
        </main>
    ...
    <script>
        ...
        function setVoiceType(type) {
            ...
            ['normal', 'funny'].forEach(v => {
                ...
                if (v === type) {
                    btn.className = "px-4 py-2 rounded-xl text-xs font-bold bg-amber-500 text-white shadow-sm";
                } else {
                    btn.className = "px-4 py-2 rounded-xl text-xs font-bold bg-slate-100 text-slate-600";
                }
            });
        }
        ...
        function renderPuzzleBoard() {
            ...
            sourceContainer.innerHTML = shuffledPuzzleWords.map((w, i) => `
                <button onclick="selectPuzzleWord('${w}', ${i}, 'source')" class="px-2.5 py-1 bg-emerald-100 active:bg-emerald-200 text-emerald-900 rounded-lg text-xs font-bold transition">${w}</button>
            `).join('');

            targetContainer.innerHTML = userPuzzleWords.map((w, i) => `
                <button onclick="selectPuzzleWord('${w}', ${i}, 'target')" class="px-2.5 py-1 bg-emerald-600 text-white rounded-lg text-xs font-bold transition">${w}</button>
            `).join('');
        }
        ...
        function checkPuzzle() {
            ...
            if (userStr === correctStr) {
                ...
                resultEl.className = "mt-2 text-xs font-bold text-emerald-600";
                setTimeout(() => {
                    nextVerse();
                    switchTab('study');
                }, 1500);
                ...
                resultEl.className = "mt-2 text-xs font-bold text-rose-500";
            }
        }
